...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_restart_drains_running_gateway_before_returning(self, monkeypatch):
        calls = []
        ...
        monkeypatch.setattr(gateway_cli.subprocess, "run", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("launchctl should not run after graceful restart")))

        gateway_cli.launchd_restart()

        assert calls == [("graceful", 321, 12.0)]

    def test_launchd_restart_falls_back_to_kickstart_when_graceful_restart_fails(self, monkeypatch):
        calls = []
    ...
    def test_launchd_restart_graceful_restart_without_kickstart(self, monkeypatch, capsys):
        calls = []
