"""Kamill regression: Discord exec approvals are mirrored to operator DMs.

Dangerous-command approvals posted only in a noisy channel/thread are easy to
miss.  Kamill keeps the original channel prompt as the canonical visible gate,
but also duplicates the same approval request to configured Discord operator
DMs (DISCORD_ALLOWED_USERS / adapter._allowed_user_ids).  A DM button resolves
the same pending approval because it carries the same session_key.
"""
