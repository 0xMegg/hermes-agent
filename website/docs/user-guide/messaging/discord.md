...
| `DISCORD_ALLOWED_USERS` | **Yes** | — | Comma-separated Discord user IDs allowed to interact with the bot. Without this **or** `DISCORD_ALLOWED_ROLES`, the gateway denies all users. Dangerous-command approval prompts are also mirrored best-effort to these numeric users by DM, while keeping the original channel/thread prompt. |
| `DISCORD_ALLOWED_ROLES` | No | — | Comma-separated Discord role IDs. Any member with one of these roles is authorized — OR semantics with `DISCORD_ALLOWED_USERS`. Auto-enables the **Server Members Intent** on connect. Useful when moderation teams churn: new mods get access as soon as the role is granted, no config push needed. The current DM mirror implementation only sends to users explicitly listed in `DISCORD_ALLOWED_USERS`; role-only authorization still works for interaction but does not add DM mirror recipients. |
...
| `DISCORD_COMMAND_SYNC_POLICY` | No | `"safe"` | Controls native slash-command startup sync. `"safe"` diffs existing global commands and only updates what changed. It preserves command IDs by default, skipping delete/recreate changes unless explicitly enabled. `"bulk"` preserves the old `tree.sync()` behavior. `"off"` skips startup sync entirely. |
| `DISCORD_COMMAND_SYNC_ALLOW_RECREATE` | No | `false` | When `true`, safe sync may delete/recreate changed commands whose fields cannot be patched. Leave disabled to avoid Discord client stale-command windows after restarts. |
| `DISCORD_COMMAND_SYNC_ALLOW_DELETE` | No | `false` | When `true`, safe sync may delete existing commands unknown to the current build. Leave disabled to preserve commands across plugin/core adapter switches and branch changes. |
